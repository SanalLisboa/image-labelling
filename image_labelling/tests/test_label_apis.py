...
class TestLabelApi:
    def test_list_image_labels_api(self, client):
        ...
        label_ids_to_consider = [str(v["id"]) for v in LABELS]
    ...
    def test_add_labels(self, client):
        ...
        data = [
            {
                "image_id": image_id,
                "coordinates": {"x1": 0.223, "y1": 3.46, "x2": 0.44564, "y2": 6.889},
                "label": "stroke",
            }
        ]
        response = client.post(
    ...
    def test_delete_label_api(self, client):
        ...
        response = client.delete(
            path
            + "?image_id={}&label_id={}".format(
                str(IMAGES[0]["image_id"]), str(labels[0].id)
            ),
            HTTP_AUTHORIZATION="JWT {}".format(login_response.data["access"]),
        )
    ...
    def test_label_search_api(self, client):
        ...
        path = router_reverse("image_labels_search")
        response = client.get(
    ...
    def test_image_label_api(self, client):
        ...
        path = router_reverse("image_labels_list")
        response = client.get(
            path,
            {"status": "all"},
            HTTP_AUTHORIZATION="JWT {}".format(login_response.data["access"]),
